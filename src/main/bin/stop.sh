...
# 죽여버리는 사고로 이어질 수 있다(실제로 발생했던 문제). 뒤에 공백까지 요구해 PNAME이 다른
# 인스턴스 이름의 접두사인 경우(예: HEDWIG_SPAM_GATEWAY가 HEDWIG_SPAM_GATEWAY_PROD의 접두사)
# 엉뚱한 인스턴스까지 같이 죽이는 사고도 방지한다(실제로 발생했던 문제).
pid=`ps -eaf | grep -E -- "-D${PNAME}[[:space:]]" | grep -v grep | awk '{print $2}'`
if [ -z "$pid" ]; then
